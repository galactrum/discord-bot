...
# region User
        def make_user(self, snowflake, address):
            to_exec = "INSERT INTO users (snowflake_pk, balance, address) VALUES(%s, %s, %s)"
            self.__cursor.execute(
                to_exec, (str(snowflake),'0', str(address)))
        ...
        def get_user(self, snowflake):
            to_exec = "SELECT snowflake_pk, balance, address FROM users WHERE snowflake_pk LIKE %s"
            self.__cursor.execute(to_exec, (str(snowflake)))
            result_set = self.__cursor.fetchone()
            return result_set

        def get_user_by_address(self, address):
            to_exec = "SELECT snowflake_pk, balance, address FROM users WHERE address LIKE %s"
